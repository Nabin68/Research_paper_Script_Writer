# Agent 5 — script REFINING brief · 2026-07-15

**Draft to refine:** Two of the world's biggest AI labs just released papers revealing the secret—  to fix AI's memory problem
- Source file: `agent5\input.md`

## ⚙️ Refine instructions for THIS pass (highest priority)

NARROW REFINE ONLY: fix the weak transition paragraph, keep everything else identical

## 📋 What Claude does next (REFINE, don't rewrite)
1. Read the draft below + the REFINER PLAYBOOK — especially §4 USER PREFERENCES, and match this draft's SUBJECT/CONTEXT against the conditional rules there before refining.
2. If a paper is attached / referenced, web-check the facts & numbers are true.
3. Refine against the page's proven levers **while keeping the user's voice and core idea** — improve, don't replace. Use your own knowledge for stronger phrasing/analogies.
4. Write the polished script to **`agent5\refined\2026-07-15-v1-two-of-the-world-s-biggest-ai-labs-just-released-p.md`** (versioned — this is **v1** for this script), ending with a short **WHAT I CHANGED & WHY** section.
5. Log this refine into `agent5/refine_playbook.md` §4B (raw log, WITH context/subject), and if the user states a preference, capture it as a CONDITIONAL rule ("IF <context> THEN <direction>") in §4A — never a flat global rule. See the playbook's §4 header.

---

# ===== YOUR DRAFT SCRIPT (refine THIS — preserve its voice/intent) =====
...
Now imagine the possibilities if we combine these both.
It solves the hard part of optimum use of memory — knowing the moment something matters, and knowing exactly where to dig for it.
Final conclusion:
...
# ===== REFINER PLAYBOOK (agent5/refine_playbook.md) — read carefully =====
...
_(no conditional rules captured yet — add them here as the user gives preferences)_
...
**EVERYDAY STAKES:**
**[the normal thing everyone accepts — plain, no jargon]**

**WHY THE OLD WAY FAILED:**
**[the dumb old behavior, concrete, a bit funny]**

**THE ONE CLEVER IDEA:**
**[the paper's switch in plain words + a human analogy; drop the single best PROOF NUMBER here]**

**WHY IT MATTERS:**
**[zoom out to the $ / job / brain / body stake]**
...
- **No timestamps** in the script (no "(0-3 sec)"). Sub-labels (EVERYDAY STAKES, etc.) are plain
  bold text with a colon.
